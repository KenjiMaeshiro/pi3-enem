...
Nas entregas anteriores, foi realizada uma etapa inicial de **mineração de textos**, utilizando artigos da Wikipédia relacionados a três portos: **Porto de Santos, Porto de Xangai e Porto de Roterdão**. O objetivo foi obter os textos, organizar os parágrafos, realizar uma tokenização simples, construir um vocabulário, analisar a frequência dos termos, criar uma matriz termo-documento e realizar buscas booleanas — etapas complementadas, na sequência, com uma **limpeza mais completa dos textos** (remoção de acentos e de pontuação), com a **remoção de stopwords**, com o cálculo do **TF-IDF** e com uma **busca por similaridade** entre a consulta e os parágrafos, utilizando a similaridade de cosseno.

Esta entrega dá continuidade a esse processo, mantendo todas as etapas anteriores e introduzindo o cálculo do **BM25**, uma métrica de ranking que evolui o TF-IDF ao incorporar a saturação da frequência dos termos e a normalização pelo tamanho dos documentos, sendo aplicada a um subconjunto do corpus.
...
Dando continuidade às etapas anteriores, esta entrega introduz o cálculo do **BM25** (Best Matching 25), uma métrica de ranking amplamente utilizada em sistemas de recuperação de informação e considerada uma evolução do TF-IDF.

Diferente do TF-IDF, o BM25 introduz dois refinamentos importantes:

- uma **saturação da frequência do termo** (controlada pelo parâmetro `k1`), de modo que a repetição excessiva de um termo em um documento deixe de aumentar seu peso indefinidamente;
- uma **normalização pelo tamanho do documento** (controlada pelo parâmetro `b`), penalizando documentos muito longos em relação ao tamanho médio do corpus.

Para esta etapa, foi selecionado um subconjunto do corpus já tokenizado e sem stopwords, sobre o qual o cálculo do BM25 foi aplicado.

Foram selecionados os **8 primeiros parágrafos** do corpus (todos pertencentes ao artigo do Porto de Santos), a partir dos quais foi construído um vocabulário próprio deste subconjunto (`vocab_8`).
...
## Cálculo do TF, |d| e a média

A partir dos tokens dos 8 parágrafos selecionados, foi construída uma matriz de frequência de termos (`tf`), alinhada ao vocabulário `vocab_8`.

A partir dessa matriz, foram calculados:

- `dl` (*document length*): o tamanho de cada documento, isto é, a quantidade total de palavras de cada parágrafo;
- `avgdl` (*average document length*): o tamanho médio dos documentos do subcorpus, usado como referência para normalizar o tamanho de cada documento no cálculo do BM25.

```r
tf <- sapply(tokens_8, function(t) as.integer(table(factor(t, levels = vocab_8)))) # Matriz de contagem
rownames(tf) <- vocab_8
dl <- colSums(tf) # |d|: quantas palavras tem cada documento
avgdl <- mean(dl) # avgdl: tamanho Médio do corpus
dl
avgdl
```

Os tamanhos obtidos para cada um dos 8 parágrafos foram:

| Parágrafo | Tamanho (`dl`) |
|---|---:|
| paragrafo-1 | 36 |
| paragrafo-2 | 38 |
| paragrafo-3 | 32 |
...
O tamanho médio dos documentos (`avgdl`) foi de **42,75** palavras. Nota-se que os parágrafos possuem tamanhos bastante distintos entre si (variando de 18 a 73 palavras), o que reforça a importância da normalização pelo tamanho do documento realizada pelo BM25.

## IDF Probabilístico

Diferente do IDF utilizado no cálculo do TF-IDF na entrega anterior, o BM25 utiliza uma versão **probabilística** do IDF, calculada da seguinte forma:

```r
N <- ncol(tf)
df <- rowSums(tf > 0)

# Versão probabilística do IDF: os "+0.5" evitam divisão/log por zero
idf <- log((N - df + 0.5) / (df + 0.5) + 1)

round(idf[c("santos", "porto", "cidade")], 3)
```

Os valores acrescidos de `0.5` no numerador e no denominador evitam divisões e logaritmos de zero, além de suavizarem o peso atribuído a termos muito frequentes ou muito raros no subcorpus.

Para os termos `santos`, `porto` e `cidade`, os pesos de IDF probabilístico obtidos foram:

| Termo | IDF probabilístico |
|---|---:|
| `santos` | 0,182 |
| `porto` | 0,182 |
| `cidade` | 1,792 |

Assim como observado no TF-IDF da entrega anterior, os termos `santos` e `porto`, por aparecerem na maioria dos 8 parágrafos selecionados, recebem um peso de IDF baixo. Já o termo `cidade`, mais raro neste subcorpus, recebe um peso consideravelmente mais alto.

## Cálculo do BM25

Por fim, foi definida a função `bm25_doc()`, que implementa a fórmula do BM25 para um conjunto de termos de consulta em relação a um documento específico.

Os parâmetros utilizados foram os valores padrão da literatura: `k1 = 1.2` (controla a saturação da frequência do termo) e `b = 0.75` (controla o peso do tamanho do documento na normalização).
...
bm25_doc <- function(termos, d) {
  ...
  for (t in termos) { # Soma sobre os termos da consulta
    if (!(t %in% vocab)) next # Se o termo não existe no vocabulário, ignora
    f <- tf[t, d] # # Frequência do termo neste parágrafo
    K <- k1 * (1 - b + b * dl[d] / avgdl) # Penalidade de tamanho
    s <- s + idf[t] * (f * (k1 + 1)) / (f + K) # Fórmula do BM25
  }
  s
}

# 2. Ordena e transforma em data frame
res <- sort(scores, decreasing = TRUE)
res_df <- data.frame(
  paragrafo = names(res),
...
Para cada termo da consulta e cada documento, a função soma o produto entre o peso `idf` do termo e a fração `f * (k1 + 1) / (f + K)`, em que `f` é a frequência do termo no documento e `K` é a penalidade aplicada em função do tamanho relativo do documento (`dl[d] / avgdl`).

O resultado exibido reaproveitou o ranking (`scores`) já calculado anteriormente por similaridade de cosseno, ordenado do maior para o menor escore:
...
A principal novidade desta entrega foi a introdução do **BM25**, calculado sobre um subcorpus formado pelos 8 primeiros parágrafos do artigo do Porto de Santos. O IDF probabilístico manteve o mesmo padrão observado no TF-IDF: termos amplamente distribuídos nesse subcorpus, como `santos` e `porto`, receberam pesos baixos (0,182), enquanto o termo mais raro, `cidade`, recebeu um peso bem mais alto (1,792). A diferença central do BM25 em relação ao TF-IDF está na função `bm25_doc()`, que passa a considerar também a saturação da frequência do termo (via `k1`) e a normalização pelo tamanho do documento em relação à média do corpus (via `b` e `avgdl`), refinando o comportamento observado no cálculo de TF-IDF e na busca por similaridade das etapas anteriores.

## Conclusão

Nesta entrega, o processo de mineração de textos foi refinado com a introdução de uma limpeza mais completa dos textos (remoção de acentos e pontuação), da remoção de stopwords e, por fim, do cálculo do **BM25**, além da manutenção das etapas de tokenização, criação do vocabulário, análise de frequência, construção da matriz termo-documento, busca booleana, cálculo do TF-IDF e busca por similaridade de cosseno.

Como resultado, o vocabulário ficou mais enxuto e mais representativo do conteúdo dos textos, a busca booleana passou a identificar corretamente mais ocorrências dos termos pesquisados, a busca por similaridade de cosseno produziu um ranking mais consistente com o tema da consulta, e o BM25 acrescentou ao processo uma métrica de ranking que leva em conta tanto a saturação da frequência dos termos quanto o tamanho relativo de cada documento, características que o diferenciam do TF-IDF utilizado nas entregas anteriores.
